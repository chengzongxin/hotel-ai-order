...
4. 当当前订单状态是 collecting 或 confirming，且用户最新输入是“取消”“不用了”“不提交”“先算了”“撤销”“放弃”“不要了”等，属于 cancel_repair_order，并且 user_cancelled 为 true。
5. 取消当前维修单时，不要复用历史维修字段生成新的维修单。
6. 用户最新输入只是闲聊、问天气、问旅游、问时间，不要复用历史订单字段。
7. 如果当前订单状态是 submitted，表示上一张维修单已经提交完成；除非用户最新输入明确提出新的报修，否则不要从历史已提交订单里重新抽取字段。
